...
namespace Modulap.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<ActionResult> DeleteUser([FromBody] DeletUserDto dto)
        {
        ...
        await _userManager.UpdateAsync(user);
            return Ok(new AuthResponseDto
            {
                ...
                Message = "Actualización del token exitosa"
...
request.AddParameter("application/json", "{\"from\":{\"email\":\"hello@demomailtrap.co\",\"name\":\"Mailtrap Test\"},\"to\":[{\"email\":\"tovardiazjuanaesmeralda@gmail.com\"}],\"template_uuid\":\"0b450eca-1f34-486a-894a-2f5db1c40cef\",\"template_variables\":{\"user_email\":\"Test_User_email\",\"pass_reset_link\":\"Test_Pass_reset_link\"}}", ParameterType.RequestBody);
...
System.Console.WriteLine(response.Content);
            */
            ...
            request.AddHeader("Authorization", "Bearer 39da691fb719936e1508d6d0d9ea6413");
            request.AddJsonBody(new
            {
                from = new {email = "mailtrap@demomailtrap.co"},
                to = new[] { new { email = user.Email } },
                template_uuid = "0b450eca-1f34-486a-894a-2f5db1c40cef",
            ...
            if (response.IsSuccessful)
            {
                ...
                {
                    IsSuccess = true,
                    Message = "Email sent with password rest link, please check your email."
                });
            }
            else
       ...
       new (JwtRegisteredClaimNames.Iss, _configuration.GetSection("JWTSetting").GetSection("ValidIssuer").Value!)
        ...
        // api/account/userdetail/{id}
        [AllowAnonymous]
        [HttpGet("userdetail/{id}")]
        public async Task<ActionResult<UserDetailDto>> GetUserDetailById(string id)
        {
        ...
        [HttpGet("admins")]
        public async Task<ActionResult<IEnumerable<UserDetailDto>>> GetAdmins()
        {
            var adminUsersList = await _userManager.GetUsersInRoleAsync("Administrador");

            // Filtrar por Estatus == 1
            var activeAdmins = adminUsersList.Where(u => u.Status == 1).ToList();
        ...
        [Authorize(Roles = "Administrador")]
        [HttpGet("cliente")]
        public async Task<ActionResult<IEnumerable<UserDetailDto>>> Getcliente()
        {
            var clienteUsersList = await _userManager.GetUsersInRoleAsync("Cliente");

            // Filtrar por Estatus == 1
            var activeClientes = clienteUsersList.Where(u => u.Status == 1).ToList();

            var result = new List<UserDetailDto>();

            foreach (var user in activeClientes)
            {
                var roles = await _userManager.GetRolesAsync(user);
        ...
        //api/account/detailClient

        [HttpGet("detailClient")]
        public async Task<ActionResult<UserDatailClientDto>> GetUserDetail1()
        {
            var currentUserId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            var user = await _userManager.FindByIdAsync(currentUserId!);
            ...
            return Ok(new UserDatailClientDto
            {
